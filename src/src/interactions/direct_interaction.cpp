    ...
    // source pairwise interactions
      for(int pair_idx = 0; pair_idx < num_srcsrc; ++pair_idx) {
        ...
        for(int i = 0; i <= interp_order; ++i) {
          ...
          // if (time_idx == 100) std::cout << "Delay: " << floor_delays[pair_idx] << std::endl;

          Eigen::Vector3d dip_src = (*dots)[src].dipole();
          Eigen::Vector3d dip_obs = (*dots)[obs].dipole();

          results[src] += std::real( (history->array_[obs][s][0])[RHO_01] ) * coeffs[pair_idx][i];
          results[obs] += std::real( (history->array_[src][s][0])[RHO_01] ) * coeffs[pair_idx][i];
