...
HistoryInteraction::HistoryInteraction(
    ...
    const std::shared_ptr<const Integrator::History<EigenVector2cd>> &history,
    const std::shared_ptr<Propagation::Kernel<cmplx>> &dyadic,
  ...
  for(int pair_idx = 0; pair_idx < num_interactions; ++pair_idx) {
    ...
    for (int i = 0; i <= interp_order; ++i) {
      coefficients[pair_idx][i] =
          ...
          (*dots)[obs].dipole_imag().dot(interp_dyads[i] * (*dots)[src].dipole_imag()) + iu * (
          (*dots)[obs].dipole().dot(interp_dyads[i] * (*dots)[src].dipole_imag()) -
          (*dots)[obs].dipole_imag().dot(interp_dyads[i] * (*dots)[src].dipole()));
    }
  }
}
