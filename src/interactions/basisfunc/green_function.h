...
//        std::cout << i << " " << interp.evaluations[0][i] << std::endl;

        if ( dr.norm() > dist0 ) {
            this->coefs_[i] = -k2_ * std::exp(-iu * omega_ * dr.norm() / c_) * (
                                dyads[0].cast<cmplx>() +
                                dyads[1].cast<cmplx>() * iu * omega_ - 
                                dyads[2].cast<cmplx>() * std::pow(omega_,2) ) * interp.evaluations[0][i];
...
*/
            ...
            if ( dr.norm() > 0.0 ){
                ...
                /*this->coefs_[i] = -k2_ * 
                    (dyads[0].cast<cmplx>() * interp.evaluations[0][i] +
                     ...
                     dyads[2].cast<cmplx>() * interp.evaluations[2][i] +
                     dyads[3].cast<cmplx>() * (
                         2.0 * iu * omega_ * interp.evaluations[1][i] - 
                         std::pow(omega_, 2) * interp.evaluations[0][i] ) );*/
            } 
            ...
            this->coefs_[i] += 
                //iu * beta_ * pow( omega_, 3 ) / pow( 5.2917721e-4, 2 ) * Eigen::Matrix3d::Identity() * interp.evaluations[0][i];
                beta_ / pow( 5.2917721e-4, 2 ) * Eigen::Matrix3d::Identity() * 
                // k2_ * 2.0 * Eigen::Matrix3d::Identity() / ( 3.0 * c_ ) * 
                    ( 0.0 * iu * pow(omega_,3) * interp.evaluations[0][i] +
                      0.0 * pow(omega_,2) * interp.evaluations[1][i] -
                      0.0 * iu * omega_ * interp.evaluations[2][i] );
//                      - interp.evaluations[3][i]) ;
        }    
    }
