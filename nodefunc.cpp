...
int node2D::isColleague(node2D* other){
    ...
    return (this!=other && lvl == other->lvl && dist <= size*sqrt(2.0));
}

int node2D::isNeighbour(node2D* other){
    double dist = abs(center - other->center);
    // double dist = sqrt(pow(real(center)-real(other->center),2) +
    //                   pow(imag(center)-imag(other->center),2));
    return (this!=other && other->isLeaf && dist <= (1+pow(2,other->lvl-lvl))*size*sqrt(2.0)/2.0);
