...
std::vector<std::vector<std::complex<double> > > node2D::coeffMpoleAll(){

    int nnodes = numNodes();
    std::vector<std::vector<std::complex<double> > > Bs(nnodes);

    for (int i=0; i<nnodes; i++ )
        Bs[i] = findNode(i)->coeffMpole;
...
void node2D::evalIList(node2D* masterNode){
    ...
    if (lvl > 1){
        ...
        for ( int j=0; j<nList.size(); j++){
            for ( int k=0; k<4; k++){
                cousin = masterNode->findNode(nList[j])->child[k];
...
/*void node2D::evalCoeffLocalExp(
    ...
    for ( int i=0; i<iList.size(); i++ ){
        ...
        for ( int l=1; l<=p; l++ ){
            B[l] = -Bs[iList[i]][0]/(l*pow(z0,l));
            for ( int k=1; k <=p; k++ )
                B[l] += Bs[iList[i]][k]*pow(-1,k)/pow(z0,k)*binomCoeffs[l+k-1][k-1];
        }
    } 
    this->coeffLocalExp = B;

    if ( !isLeaf ){
        child[0]->evalCoeffLocalExp(Bs, p);
        child[1]->evalCoeffLocalExp(Bs, p);
        child[2]->evalCoeffLocalExp(Bs, p);
        child[3]->evalCoeffLocalExp(Bs, p);
    }
}
