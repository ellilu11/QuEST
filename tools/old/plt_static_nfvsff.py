...
fldRaw = np.genfromtxt( \
    dirstr+'fld_dir_ff.dat',delimiter=' ', dtype=None, usecols=range(0,3*nparts) )
...
# Get analytic fld
fldRaw = np.genfromtxt( \
    dirstr+'fld_dir_nf.dat',delimiter=' ', dtype=None, usecols=range(0,3*nparts) )

fldNf = np.zeros( (nsteps, 3, nparts) )
fldAbsNf = np.zeros( (nsteps, nparts) )

if nparts > 1:
    for step in range(nsteps) :
        for part in range(nparts) :
            fldNf[step,0,part] = fldRaw[step,3*part]
            fldNf[step,1,part] = fldRaw[step,3*part+1]
            fldNf[step,2,part] = fldRaw[step,3*part+2]
            fldAbsNf[step,part] = LA.norm(fldNf[step,:,part])
...
for i in range(1,7) :
    ...
    if i < 4 :
        plt.plot( tdata, fld[:,i-1,part], tdata, fldNf[:,i-1,part])
        plt.title(titlestr[i-1])
    else :
        plt.plot( tdata, fldNf[:,i-4,part] - fld[:,i-4,part])
