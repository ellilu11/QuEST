...
*/
    ...
    *obs = *qds; // examine field at sources only
    ...
    for(int t = 0; t < num_timesteps; ++t) {
      ...
      auto fldx = interactions_fld[interactions_fld.size()-1]->evaluate(t); 
        // std::accumulate(
        //    interactions_fld.begin(), interactions_fld.end(), nilfld, eval_and_sum);  
      
      set_dipolevec(obs, Eigen::Vector3d(0,hbar,0));
      auto fldy = interactions_fld[interactions_fld.size()-1]->evaluate(t); 
        //std::accumulate(
        //    interactions_fld.begin(), interactions_fld.end(), nilfld, eval_and_sum);  

      set_dipolevec(obs, Eigen::Vector3d(0,0,hbar));
      auto fldz = interactions_fld[interactions_fld.size()-1]->evaluate(t); 
        //std::accumulate(
        //    interactions_fld.begin(), interactions_fld.end(), nilfld, eval_and_sum);  
