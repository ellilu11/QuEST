...
std::vector<double>
    ...
    int ndots = (*dots).size();
    std::vector<double> fld_anlytc(ndots);
    double dist, delay;
    ...
    for (int itrg = 0; itrg < ndots; itrg++) {
        for (int isrc = 0; isrc < ndots; ++isrc){
            if ( itrg != isrc ) {
                ...
                Eigen::Vector3d efld_d0( gaussian((t-delay-mu) / sigma), 0, 0) ;
...
int main(int argc, char *argv[]){
   ...
   // set up dots and history of dots
    ...
    dots->emplace_back(Eigen::Vector3d(10.5, 0.6, 0.7), Eigen::Vector3d(dip, 0, 0), Eigen::Vector3d(0, 0, 0));
    ...
    std::vector<double> fld_anlytc( ndots );
    std::vector<double> fld_dir ( ndots );
    double normdiff_dir = 0;
    for (int i = 0; i < steps; ++i) {
        
        fld_anlytc[i] = analytic_evaluate( dots, i );

        fld_dir = direct_interaction->evaluate(i);
