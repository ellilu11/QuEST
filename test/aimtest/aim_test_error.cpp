    ...
    */
    ...
    for (int i = 0; i < steps; ++i) {
        ...
        for (int itrg = 0; itrg < ntrgs; ++itrg){
            ...
            fld_dir[i][itrg] = array_dir[nsrcs+itrg] * hbar / (*dots)[nsrcs+itrg].dipole().norm();
        }
    }
    ...
    std::vector<std::vector<std::complex<double>>> fld_aim(
        steps, std::vector<std::complex<double>>(ntrgs));
    ...
    for (int i = 0; i < steps; ++i) {
        ...
        for (int itrg = 0; itrg < ntrgs; itrg++) 
            fld_aim[i][itrg] = array_aim[nsrcs+itrg] * hbar / (*dots)[nsrcs+itrg].dipole().norm();
    ...
    for (int itrg = 0; itrg < ntrgs; ++itrg){
        for (int i = 0; i < steps; ++i){
            outfile1 << real(fld_dir[i][itrg]) << " " << real(fld_anlytc[i][itrg]) << " " << real(fld_dir[i][itrg]) / real(fld_anlytc[i][itrg]) << std::endl; 
            normdiff_dir += pow( real(fld_dir[i][itrg]) - real(fld_anlytc[i][itrg]), 2);

            outfile2 << real(fld_aim[i][itrg]) << " " << real(fld_anlytc[i][itrg]) << " " << real(fld_aim[i][itrg]) / real(fld_anlytc[i][itrg]) << std::endl; 
            
            normdiff_aim += pow( real(fld_aim[i][itrg]) - real(fld_anlytc[i][itrg]), 2);
            normdiff_aimdir += pow( real(fld_aim[i][itrg]) - real(fld_dir[i][itrg]), 2);
