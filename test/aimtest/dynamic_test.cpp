...
int main(int argc, char *argv[])
{
    ...
    const double dt = 0.5e-2; // rotframe: sigma = 1.0ps -> dt <= 0.52e-1
    ...
    double beta = atoi(argv[4]) ? 0.5 * pow(2, atoi(argv[4]) - 1) : 0.0;
    // * 1.2432e-04
    ...
    history->initialize_past( Eigen::Vector2cd(1,0), num_particles );
...
*/
    ...
    /*ofstream rhofile2("rho_smpl.dat");
    rhofile << scientific << setprecision(15);

    ofstream fldfile2("rabi_smpl.dat");
    fldfile << scientific << setprecision(15);

    ofstream posfile2("pos_smpl.dat");
    posfile << scientific << setprecision(15);*/
    ...
    /*const double xlen = 16*ds, ylen = 16*ds, zlen = 0.030*num_particles*ds;
    Eigen::Vector3d rlen(xlen, ylen, zlen);
    const int nx = 2, ny = 2, nz = 12;
    ...
    std::vector<double> ipr2(num_timesteps);
     ...
     for(int t = 0; t < num_timesteps; ++t) {
      ...
      for(int n = 0; n < num_particles; ++n) {
        ...
        // fldfile << std::abs(history_efld->array_[n][t][0][0]) << " " ;
